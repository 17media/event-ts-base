...
`).to('prettier.config.js');
    ...
    write_1.default(yield fs_1.readFile(`node_modules/${package_json_1.default.name}/tsconfig.json`)).to('tsconfig.base.json');
    if (tsconfigs.length) {
        console.log(`${chalk_1.default.cyan('tsconfig.json')} is already exist.`);
    }
    else {
        write_1.default(JSON_1.stringify({ extends: `./tsconfig.base` })).to('tsconfig.json');
    }
    ...
    Object.assign(pkg['lint-staged'], package_json_1.default['lint-staged']);
